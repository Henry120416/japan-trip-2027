...
if (USE_PG) {
  ...
  initDB = async () => {
    ...
    // ── 方案一費用種子（完整版，含機票/住宿/餐費/交通，強制重置）──────
    const p1ec = await pool.query(`SELECT COUNT(*) as c FROM expenses WHERE day_id IN (SELECT id FROM days WHERE plan_id=1)`);
    if (parseInt(p1ec.rows[0].c) < 20) {
      await pool.query(`DELETE FROM expenses WHERE day_id IN (SELECT id FROM days WHERE plan_id=1)`);
      // 格式：[date, title, amount_jpy, amount_twd, payer, notes]
      const P1_EXP = [
        // 機票（純台幣）
        ['2027-04-17','來回機票（台灣↔關西）×6',          0, 48000, '', '預估NT$8,000/人，視航空公司調整'],
        // 住宿
        ['2027-04-17','京都住宿・4晚（3間）',         144000, 30000, '', '預估¥12,000/間/晚 × 3間 × 4晚'],
        ['2027-04-21','大阪住宿・1晚（3間）',          33000,  6875, '', '預估¥11,000/間/晚 × 3間'],
        // D1：4/17
        ['2027-04-17','HARUKA電車（KIX→京都）×6',    21600,  4500, '', '3,600円/人，單程；建議搭配ICOCA套票'],
        ['2027-04-17','先斗町壽喜燒/京料理晚餐×6',   57600, 12000, '', '預估9,600円/人；先斗町料亭含飲料，高級區域'],
        ['2027-04-17','京都市區公車・地鐵（3日）×6', 12600,  2625, '', '700円/人/日 × 3天'],
        // D2：4/18
        ['2027-04-18','JR宇治線往返×6',               3480,   725, '', '290円/人單程；往返580円'],
        ['2027-04-18','平等院鳳凰堂入場費×6',          3600,   750, '', '600円/人'],
        ['2027-04-18','中村藤吉抹茶午餐×6',           18000,  3750, '', '預估3,000円/人；名店抹茶套餐含甜點'],
        ['2027-04-18','東寺入場費×6',                  3000,   625, '', '500円/人'],
        ['2027-04-18','伊勢丹美食街晚餐×6',           18000,  3750, '', '預估3,000円/人；京都車站B1~B2百貨美食'],
        // D3：4/19
        ['2027-04-19','清水寺入場費×6',                3000,   625, '', '500円/人'],
        ['2027-04-19','石板路午餐・京都定食×6',        15000,  3125, '', '預估2,500円/人；三年坂/清水坂觀光區行情'],
        ['2027-04-19','京都和牛燒肉晚餐×6',            48000, 10000, '', '預估8,000円/人；木屋町通和牛燒肉含飲料'],
        // D4：4/20
        ['2027-04-20','嵐山竹林午餐×6',               15000,  3125, '', '預估2,500円/人；嵐山渡月橋周邊觀光餐廳'],
        ['2027-04-20','金閣寺入場費×6',                3000,   625, '', '500円/人'],
        ['2027-04-20','仁和寺入場費×6',                3000,   625, '', '500円/人'],
        ['2027-04-20','行李宅配（京都→大阪）',          3000,   625, '', '1,500円/件 × 2件'],
        ['2027-04-20','JR京都→大阪新快速×6',           4380,   913, '', '730円/人'],
        ['2027-04-20','道頓堀晚餐・串炸/章魚燒×6',    18000,  3750, '', '預估3,000円/人；串炸+章魚燒+飲料'],
        // D5：4/21
        ['2027-04-21','阪神電車往返（大阪↔神戶三宮）×6',3960,  825, '', '330円×2/人，梅田出發'],
        ['2027-04-21','北野異人館入場費×6',             4800,  1000, '', '預估800円/人（風見鶏の館等）'],
        ['2027-04-21','頂級神戶牛鐵板燒午餐×6',        72000, 15000, '', '預估12,000円/人；頂級鐵板燒，神戶A5和牛'],
        ['2027-04-21','神戶港灣・美利堅公園茶點×6',     9000,  1875, '', '預估1,500円/人；港灣咖啡廳飲料甜點'],
        ['2027-04-21','摩耶山ロープウェイ往返×6',      10200,  2125, '', '1,700円/人往返'],
        // D6：4/22
        ['2027-04-22','黑門市場早餐・海鮮×6',          18000,  3750, '', '預估3,000円/人；觀光客價位，生魚片/海鮮燒烤'],
        ['2027-04-22','臨空城午餐・輕食×6',            12000,  2500, '', '預估2,000円/人'],
        ['2027-04-22','南海電車（難波→KIX）×6',         6120,  1275, '', '1,020円/人'],
      ];
