...
// ── 方案二行程資料（京都出發・神戶一日遊）─────────────
const PLAN2_DAYS = [
  { date:'2027-04-17', title:'京都初抵・鴨川祇園',          city:'京都',      notes:'下午悠閒抵達，感受京都的第一個黃昏，漫步鴨川與祇園白川。[t1]' },
...
if (USE_PG) {
  ...
  initDB = async () => {
    ...
    if (parseInt(dc.rows[0].c) === 0) {
      ...
      if (!p1title.includes('京都初抵') || !p1notes.includes('[t1]')) {
        for (const d of P1_DAYS) {
