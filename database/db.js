...
// ── 方案二行程資料（京都出發・神戶一日遊）─────────────
const PLAN2_DAYS = [
  { date:'2027-04-17', title:'京都慢慢開始',             city:'京都',      notes:'從關西機場搭 HARUKA 抵達，入住 Minn 祇園三條，漫步白川與鴨川，先斗町晚餐。[t2]' },
...
const PLAN2_ACTS = [
  ...
  ['2027-04-17',2,'15:00','transport','抵達京都站・入住 Minn 祇園三條','京都駅（JR）',34.9858,135.7585,
   '▶ 從京都車站步行15分鐘：出中央口→河原町通北上→三条大橋，Minn 祇園三條在三条大橋旁。\n\n入住後放下行李，準備探索祇園。Minn 祇園三條地點絕佳，步行即可抵達白川・鴨川。'],
  ['2027-04-17',3,'16:45','attraction','白川巽橋散步','祇園白川（巽橋周邊）',35.0037,135.7752,
   '▶ 從 Minn 祇園三條步行5分鐘：往東沿三条通過巽橋即達白川沿岸。\n\n白川沿岸垂柳輕拂，老町家緊鄰水畔，傍晚燈籠亮起更顯古都氛圍。巽橋是拍攝京都夜景的經典機位。'],
  ['2027-04-17',4,'17:30','attraction','鴨川夕陽','鴨川（三条〜四条段）',35.0042,135.7704,
   '▶ 從白川巽橋步行8分鐘往西：沿三条通往西過三条大橋即達鴨川。\n\n京都最療癒的散步路線。三条至四条段兩岸設有台階，傍晚時分本地人在此野餐談天，感受京都日常。'],
  ['2027-04-17',5,'18:45','food','先斗町晚餐','先斗町（木屋町通）',35.0034,135.7697,
  ...
  // Day 4 — 京都黃金時段→移大阪
  ['2027-04-20',1,'05:50','transport','搭計程車出發','飯店出發（Minn 祇園三條）',35.0055,135.7742,
  ...
  ['2027-04-20',11,'12:30','transport','京都→大阪（JR）','京都駅',34.9858,135.7585,
   '▶ 從Minn 祇園三條搭地下鐵東西線→烏丸御池→烏丸線→京都駅（10分，260円），搭JR京都線新快速→大阪駅（28分，570円）。\n\n新快速不需預約，每10-15分鐘一班。到大阪駅後搭御堂筋線（梅田→難波，2分，180円）。'],
  ['2027-04-20',12,'14:00','transport','入住 Minn 難波 South','Minn 難波 South',34.6655,135.5030,
   '▶ 從難波駅步行5-8分鐘：出地下鐵難波駅往南，Minn 難波 South 位於難波中心。\n\n地理位置優越，步行即達道頓堀・心齋橋・黑門市場。辦理入住後稍作休息。'],
  ...
  ['2027-04-21',1,'08:30','transport','搭阪神電車往神戶','難波（大阪Metro→梅田）',34.7024,135.4959,
   '從難波搭大阪Metro御堂筋線→梅田（2分，180円），轉阪神電車「直通特急」→神戶三宮（35分，430円）。08:30出發，10:00抵達三宮。'],
  ...
  ['2027-04-21',6,'17:30','attraction','摩耶山掬星台・百萬夜景','摩耶山掬星台（まや観光ロープウェイ）',34.7167,135.2125,
   '▶ 從Harborland搭神戶市巴士18號→「摩耶ケーブル」（25分，230円）→摩耶山ケーブル（6分，870円）→轉まやロープウェイ→掬星台（6分，870円）。末班上行約21:30，請提前確認。\n\n日本三大夜景之一！標高690公尺俯瞰大阪灣，神戶・大阪・六甲山脈盡收眼底。'],
  ['2027-04-21',7,'20:00','transport','返回大阪','神戶三宮駅',34.6913,135.1956,
   '▶ 下摩耶山後搭市巴士回三宮→阪神電車或阪急→大阪梅田→御堂筋線→難波。約20:00返抵大阪飯店。'],
  // Day 6 — 臨空城 Outlet・返程
  ['2027-04-22',1,'08:30','transport','飯店退房','Minn 難波 South',34.6655,135.5030,
   '整理行李，完成退房手續。大件行李可存放飯店（若班機較晚），或帶至南海難波駅寄放投幣置物箱。今日行程輕鬆，無需趕時間。'],
...
if (USE_PG) {
  ...
  initDB = async () => {
    ...
    if (parseInt(dc.rows[0].c) === 0) {
      ...
      if (!p1title.includes('京都慢慢開始') || !p1notes.includes('[t2]')) {
        for (const d of P1_DAYS) {
