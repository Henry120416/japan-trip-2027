...
// ── 方案二行程資料（京都出發・神戶一日遊）─────────────
const PLAN2_DAYS = [
  { date:'2027-04-17', title:'京都慢慢開始',             city:'京都',      notes:'從關西機場搭 HARUKA 抵達，入住 Minn 祇園三條，漫步白川與鴨川，先斗町晚餐。[t4]' },
...
const PLAN2_ACTS = [
  ...
  ['2027-04-17',2,'15:00','transport','抵達京都駅（JR 中央口）','京都駅 JR 中央口',34.9858,135.7585,
   '出 JR 中央口後往北，沿烏丸通→河原町通方向步行約15分鐘；或搭市巴士5號/17號→「三条京阪」下車（10分，230円），步行3分至飯店。'],
  ...
  ['2027-04-18',0,'07:30','hotel','從 Minn 祇園三條 出發','Minn 祇園三條（東山区三条通）',35.0065,135.7742,
   '從飯店步行3分鐘至三条京阪駅，搭京阪電車→京都駅，轉近鐵京都線「急行」前往奈良。'],
  ...
  ['2027-04-19',0,'07:30','hotel','從 Minn 祇園三條 出發','Minn 祇園三條（東山区三条通）',35.0065,135.7742,
   '從飯店步行3分鐘至三条京阪駅，搭京阪電車→出町柳（5分，210円），轉叡山電鐵前往貴船。'],
  ...
  ['2027-04-21',0,'08:15','hotel','從 Minn 難波 South 出發','Minn 難波 South（千日前2丁目）',34.6625,135.5010,
   '從飯店步行5分鐘至地下鐵難波駅，搭御堂筋線→梅田（2分，180円），轉阪神電車「直通特急」前往神戶三宮。'],
...
if (USE_PG) {
  ...
  initDB = async () => {
    ...
    if (parseInt(dc.rows[0].c) === 0) {
      ...
      if (!p1title.includes('京都慢慢開始') || !p1notes.includes('[t4]')) {
        for (const d of P1_DAYS) {
